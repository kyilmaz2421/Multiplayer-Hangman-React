...
export default function Lobby() {
  ...
  useEffect(() => {
    socket.on("receive-invite", ({ invitation }) => {
      if (!showPopup.acceptedInvite) {
        // or in game -- need to set that up
  ...
  useEffect(() => {
    socket.on(
      "remove-from-lobby",
      ({ hostName, userToRemove, senderSocketLeaveTo }) => {
        console.log(
          "YO",
          userToRemove,
          invitedUsers,
            ...
            >
              ...
              <InputGroup.Append>
                <Button
                  ...
                  onClick={async () => await inviteNewUser()}
                >
              ...
              <Card>
                <Card.Body>
                  {invitedUsers.map((user, i) => (
                    <Button
                      key={i}
                      onClick={() => removeUserFromLobby(i, false)}
              ...
              <Card>
                <Card.Body>
                  {usersInLobby.map((user, i) => (
                    <Button
                      key={i}
                      onClick={() => removeUserFromLobby(i, true)}
